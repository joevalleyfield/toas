...
def test_api_stream_read_async_step_op_delegates_to_backing_store():
    run = api.AsyncRun(run_id="api-rs-op", workdir="/tmp", process=None)
    with run.lock:
        run.output = "abcdef"
        run.status = "running"
        api.emit_stream_event(run, "llm_delta", {"text": "ab"})
    ...
    out = api.stream_read_async_step_op(
        {
            "run_id": "api-rs-op",
            "mode": "poll",
            "since_seq": 0,
