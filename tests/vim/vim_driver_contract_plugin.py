...
def run(vim_bin: str, scenario: str, timeout_s: float, speed: str) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix='toas-stdio-plugin-') as td:
        ...
        script.write_text("\n".join([
            ...
            "set noswapfile",
            "set shortmess+=I",
            f"let g:toas_stdio_contract_host = 'python3 {str((root / 'tests' / 'vim' / 'stdio_contract_host_service.py')).replace("'","''")}'",
            f"source {str((root / 'vim' / 'plugin' / 'toas_stdio_contract.vim')).replace("'","''")}",
            f"call writefile(['exists=' . exists('*ToasStdioContractRunFn')], '{str(dbg).replace("'","''")}')",
            "let g:r = -1",
            "let g:e = ''",
            "try",
            ...
            "endtry",
            f"call writefile(['type=' . type(g:r), 'string=' . string(g:r)], '{str(dbg).replace("'","''")}', 'a')",
            f"call writefile(['err=' . g:e], '{str(dbg).replace("'","''")}', 'a')",
            f"call writefile([json_encode(g:r)], '{str(out).replace("'","''")}')",
            "qa!",
        ]) + "\n", encoding='utf-8')
