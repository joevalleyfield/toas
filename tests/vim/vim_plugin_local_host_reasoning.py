...
class _FakeReasoningHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        reasoning_payload = {
            ...
            "model": "fake-model",
            "choices": [{"index": 0, "delta": {"reasoning_content": "Contemplating this\n"}, "finish_reason": None}],
        }
        answer_payload = {
            "id": "chatcmpl-reasoning",
            "object": "chat.completion.chunk",
        ...
        self.wfile.write(f"data: {json.dumps(reasoning_payload)}\n\n".encode("utf-8"))
        self.wfile.flush()
...
def _start_fake_llm_server() -> tuple[ThreadingHTTPServer, threading.Thread, str]:
    server = ThreadingHTTPServer(("127.0.0.1", 0), _FakeReasoningHandler)
...
def run(vim_bin: str, timeout_s: float) -> dict[str, object]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="toas-vim-reasoning-") as td:
            tdp = Path(td)
            ...
            ok = (
                ...
                and "## TOAS:THINKING" in result.get("text", "")
                and "Contemplating this" in result.get("text", "")
                and "## /TOAS:THINKING" in result.get("text", "")
