...
/prompt session-start/backend-deconfliction
/prompt session-start/protocol-entrainment
/prompt session-start/role-framing
/prompt session-start/start-here
/prompt session-start/templates
...
Implement /help config support in this TOAS repository. Keep help output compact, include usage guidance, and mention /config values <key>. Return YAML operations only.
## TOAS:ASSISTANT
...
- operation: shell_script
  arguments:
    script: |
      # This is a placeholder for the actual implementation steps.
      # 1. Locate the config handling logic in the codebase.
      # 2. Add a new command handler for '/help config'.
      # 3. Implement the logic to reflect current 'llm.model', 'extraction.*', etc.
      # 4. Ensure output is compact and includes usage guidance.
      echo "Implementation plan: Search for config management and inject /help config handler."
- operation: search
  arguments:
    query: "config"
```
