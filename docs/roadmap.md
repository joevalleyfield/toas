...
- `563` Vim buffer-aware surface/session auto-selection opened: plugin payload shaping now derives transcript session from active TOAS buffer for `step_async*`/`step` starts with explicit-override precedence and parity test assertions, pending full Vim harness validation
