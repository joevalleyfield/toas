...
- Active: `260624-large-event-graph-render-performance` focuses the shared event graph renderer on large `.toas/events.jsonl` files after a 55MB log made `toas graph` effectively unusable.
- Active: `260624-message-timestamps-and-toas-provenance` adds per-message epoch timestamps and sparse TOAS writer provenance for durable-history forensics.
