...
- `665` shell-intent streaming backslide (buffered-drain instead of live deltas) in progress: source emission parity is restored and Vim/workdir pathing issues were corrected, but throughput/cadence tuning remains active; current bottleneck has shifted to subscribe-window churn + watch tick pacing under high-volume tool-lane output.
