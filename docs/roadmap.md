...
- `666` de-risking slice in progress: async in-process shell stdout streaming now threads an explicit `stream_stdout_enabled` value through operator/CLI/step seams, removing worker-level `TOAS_STREAM_STDOUT` mutation while leaving remaining LLM/reasoning/progress env decoupling visible.
