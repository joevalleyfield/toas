...
| Model Invocation vs Model Backend Lifecycle | Invocation owns provider calls/request shaping/response normalization; lifecycle owns managed-local process state and health only when TOAS started the backend | Both share backend/model config and failures can appear together operationally | Split keeps live process state out of model-call audit state | Provider failure is not automatically lifecycle failure | Model calls and managed-local process lifecycle can be tested independently | Use `Model Invocation` and `Model Backend Lifecycle` | Keep split |
