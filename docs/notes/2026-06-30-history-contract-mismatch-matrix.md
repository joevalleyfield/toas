...
| `history_lines()` in `src/toas/operator_api.py` | now defaults hot/current and renders one root-to-head lineage window from the hot event log | readable progression surfaces should be hot-first by default; stitched or broader history should be explicit | `independent_hot_root_after_rotation`; `aligned_cold_hot_continuation`; `ambiguous_same_local_id_across_sources` | resolved by `260703-history-hot-default`; future selector/stitch modes remain separate |
| `transcript_text()` in `src/toas/operator_api.py` | now defaults hot/current and projects a transcript-shaped selected lineage from the hot event log | transcript projection is lineage-shaped and must not stitch by raw id; broader/stitched transcript modes should be explicit | `ambiguous_same_local_id_across_sources`; `aligned_cold_hot_continuation` | resolved by `260703-transcript-llm-input-hot-default`; future stitcher/equivalence work remains separate |
| `llm_input_messages()` in `src/toas/operator_api.py` | now defaults hot/current and derives provider/model-input projection from the same hot lineage as transcript | provider identity is derived and may differ from transcript without mutating durable history; broader/stitched LLM-input modes should be explicit | `non_message_facts_across_scopes`; `ambiguous_same_local_id_across_sources`; `aligned_cold_hot_continuation` | resolved by `260703-transcript-llm-input-hot-default`; envelope/provider scenario tests remain useful follow-ons |
