...
def handle_config_help_commands(command: str, args: list[str], *, step_mod, context: OperatorCommandContext) -> list[dict] | None:
    if command == "config":
        ...
        if args[0] == "backend":
            ...
            if sub == "add":
                ...
                return [{"role": "result", "content": f"added backend {backend_id}", "config_update": {"llm": {"backends": backends_updated}}}]
            ...
            if sub == "remove":
                ...
                backend_id = args[2].strip()
                backends_updated = [b for b in _backend_list_dicts() if b["id"] != backend_id]
                return [{"role": "result", "content": f"removed backend {backend_id}", "config_update": {"llm": {"backends": backends_updated}}}]
            ...
            if sub == "set":
                ...
                return [{"role": "result", "content": f"updated backend {backend_id}.{field}", "config_update": {"llm": {"backends": backends_updated}}}]
        ...
        raise ValueError(
            "usage: /config [show] [--sources] | /config set <key> <value> | /config unset <key> | /config restore | /config load [path] | /config save [path] | /config secret ..."
        )
