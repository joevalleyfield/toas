...
def handle_prompt_workspace_commands(command: str, args: list[str], *, step_mod, context: OperatorCommandContext) -> list[dict] | None:
    if command == "prompts":
        if len(args) > 1:
    ...
    if command == "shell":
        if args and args[0] == "config":
            if len(args) < 2 or args[1] == "list":
                baseline = tuple(
                    sorted(step_mod.normalize_shell_grants(context.config.shell.allowed_commands if context.config.shell.allowed_commands else step_mod.SHELL_ALLOWED))
                )
            ...
            if sub in {"add", "remove"}:
                ...
                baseline = set(
                    step_mod.normalize_shell_grants(context.config.shell.allowed_commands if context.config.shell.allowed_commands else step_mod.SHELL_ALLOWED)
                )
                if sub == "add":
                    baseline.add(grant)
                else:
        ...
        if not args or args[0] == "list":
            effective, baseline, sources, transcript_added, transcript_removed = step_mod._resolve_shell_grants_with_sources(context.working, context.config)
            return [
                {
                    "role": "result",
    ...
    if command == "cd":
        ...
        if raw_target == "-":
            ...
            target = Path(context.previous_command_cwd).expanduser().resolve()
        else:
