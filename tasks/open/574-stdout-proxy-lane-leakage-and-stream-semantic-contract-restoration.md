...
- 2026-05-31: Follow-up taxonomy fix: `llm_done` is no longer the generic activity terminator for non-LLM work. Runs without LLM activity now finalize with `run_done` on the `run` lane; LLM activity still finalizes with `llm_done` on `llm_answer`. Vim also stops advancing backend `since_seq` from synthetic `compat` lane events so compat terminal projections cannot skip real `tool_done`/`run_done` events after resubscribe.
