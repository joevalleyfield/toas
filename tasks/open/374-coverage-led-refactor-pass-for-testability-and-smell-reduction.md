...
- normalized replay-script artifact status-line path rendering to POSIX separators (`Path.as_posix()`), fixing Windows-only assertion drift in `test_run_replay_script_local_dry_run_writes_artifact_and_skips_step` without changing artifact write semantics
