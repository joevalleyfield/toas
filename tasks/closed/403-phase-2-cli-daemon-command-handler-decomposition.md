...
- post-migration coverage push landed for daemon package modules:
  - `daemon/__init__.py` raised to `92%`
  - `daemon/process_control.py` raised to `94%`
  - targeted branch tests added in `tests/test_daemon_package_coverage.py` for `serve_forever`, `start`, `stop`, `main`, and process-control windows branch paths
