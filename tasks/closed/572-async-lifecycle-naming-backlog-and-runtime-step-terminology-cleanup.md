...
- 2026-05-31: Phase C slice 1 landed (compatibility-safe convergence): added external CLI aliases `service -> daemon` and `transport -> host`, plus help/usage wording updates that clarify alias intent without breaking existing command surfaces.

## Outcome

- Closed.
- Naming ownership inventory/classification is documented and stitched to roadmap context.
- Phase A terminology guardrails are landed (`docs/capabilities.md`, `docs/terminology.md`).
- Phase B seam-first rehome is landed:
  - async activity store implementation now runtime-owned (`runtime/async_activity_store_impl.py`)
  - async step worker implementation now runtime-owned (`runtime/async_step_runtime_worker.py`)
  - daemon paths (`daemon/run_store.py`, `daemon/async_runner.py`) are explicit compatibility aliases with sunset criteria
- Phase C compatibility-safe external convergence started with non-breaking CLI aliases:
  - `service` -> `daemon`
  - `transport` -> `host`
