...
3. ~~Add a local `toas backend ...` path in `cli_async_commands.run_backend` for `TOAS_RPC_MODE=off` / local mode, keeping RPC available as compatibility transport.~~ Done: `run_backend` now branches on `deps.rpc_enabled_for_call()` — RPC-enabled path unchanged, local path calls `_run_backend_local` which constructs a fresh `ModelBackendLifecycle` per invocation (correct for short-lived CLI processes; managed-local state is ephemeral per-call, which is the documented unresolved keying question). `test_run_backend_requires_rpc_enabled` replaced with `test_run_backend_local_path_when_rpc_disabled` exercising `external` mode through the local path.
4. ~~Decide whether stdio host should enable all backend lifecycle ops immediately or stage status first; if staged, make unavailable mutations explicit in task-space.~~ Done: all four ops enabled immediately. `cli_host_commands.py` gains a module-level `_HOST_BACKEND_LIFECYCLE = ModelBackendLifecycle(...)` and passes the four `managed_backend_*_fn` lambdas into `build_local_request_handler_runtime`, matching the daemon pattern. No staging: domain module already covers active-runs blocking for stop/restart, and the local CLI path (step 3) already exercises all four ops.
5. ~~Retarget tests from daemon internals to runtime lifecycle ownership, leaving daemon facade tests focused on adapter parity.~~ Done: `test_daemon.py` backend lifecycle tests now manipulate `daemon._BACKEND_LIFECYCLE._state.proc` directly (no internal daemon state), verify adapter routing through `_managed_backend_*` wrappers, and monkeypatch `daemon._BACKEND_LIFECYCLE._has_active_runs` for isolation. Domain contract tests live in `test_model_backend_lifecycle.py` (34 tests). Deleted `test_daemon_backend_lifecycle.py` and `test_daemon_facade_backend_state_ops.py` as part of step 2.
