...
- Implemented template migration:
  - added file-backed template assets:
    - `src/toas/prompts/dynamic/capabilities/overview_v1.txt`
    - `src/toas/prompts/dynamic/capabilities/repo-work_v1.txt`
    - `src/toas/prompts/dynamic/capabilities/start-here_v1.txt`
  - refactored `src/toas/capability_prompts.py` to load template bodies from disk and populate runtime fields (`tool_lines`, `shape_lines`, shell limits, profile/policy interpolation).
  - retained dynamic prompt refs and runtime interpolation behavior while removing hardcoded body-template prose from Python.
  - updated prompt asset listing behavior to avoid duplicate `dynamic/capabilities/*` entries when static files are present (`src/toas/prompts.py`).
- Validation:
  - `uv run pytest tests/test_prompts.py -q --no-cov` passed
  - `uv run pytest -q` passed (`1270 passed`, coverage `93.01%`).
